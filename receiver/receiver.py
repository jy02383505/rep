...
logfile = '/Application/rep/logs/receiver.log'
...
class Application(tornado.web.Application):
    ...
    def __init__(self):
        handlers = [
            (r"/receiveStore", MainHandler),
            (r"/preload_report", PreloadReport),
        ]
...
class MainHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        try:
            ...
            # rcv_message = "hellworld"
            # print self.request.body
            data = json.loads(self.request.body)

            logging.info('post message body data is {0}'.format(data))
            #ip = self.request.remote_ip
            #vip = data.get('vip', ip)
            #vip = data.get('vip',False)

            #real_ip = self.request.headers.get('X-Real-Ip')
            #vip = data.get('vip', real_ip)
            if not data.get('vip'):
                logging.info('error devices is {0}:{1}'.format(
                    data.get('hostname'), data.get('vip')))

            if data.get('vip') and data.get('vip') != 'global':
                vip = data.get('vip')
            else:
                real_ip = self.request.headers.get('X-Real-Ip')
                vip = real_ip
            ...
            new_dict = {'host': vip, 'hostname': hostname, 'time': now}
            ...
            #    channel.basic_publish(exchange='',routing_key='result_task',body=json.dumps(message),properties=pika.BasicProperties(content_type="text/plain", delivery_mode=1))
            if isinstance(result, list):
                for message in result:
                    # rcv_message =  data['message']
                    # print rcv_message

                    print("--------------------------------------------------------------")
                    message.update(new_dict)
                    message['session_id'] = message.pop('id')
                    print(message)

                    # logging.info('About to send received message to rabbitmq exchange %s', rcv_message)
                    # print channel
                    # channel.basic_publish(exchange='', routing_key='result_task', properties=pika.BasicProperties(content_type='application/text'), body=rcv_message)
                    channel.basic_publish(exchange='', routing_key='result_task', body=json.dumps(message),
                                          properties=pika.BasicProperties(content_type="text/plain", delivery_mode=1))
                    logging.info('post message  is {0}'.format(message))
            else:
                message = data
                message.update(new_dict)
                message['session_id'] = message.pop('id')
            ...
            #logger.error('Something went wrong... %s', e)
            logger.info('put message to rabbitmq error {}'.format(traceback.format_exc(e)))
...
class PreloadReport(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        try:
            ...
            channel.basic_publish(exchange='', routing_key='preload_report', body=json.dumps(report_body),
                                  properties=pika.BasicProperties(content_type="text/plain", delivery_mode=1))
            self.write('ok')
        except Exception:
