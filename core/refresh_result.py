...
class Refresh_router(object):
    ...
    def refresh_router(self, queue_name='result_task'):
        ...
        logger.debug('copy_message_refresh action {}'.format(copy_message_refresh ))
        for key_db_k in copy_message_refresh:
            #logger.debug('merged_refresh for in {}'.format(key_db_k))
            logger.debug('merged_refresh for in {}'.format(copy_message_refresh.get(key_db_k)))
            self.update_refresh_result2(copy_message_refresh.get(key_db_k))
        else:
            logger.debug('self.merged_refresh else {}'.format(self.merged_refresh))
        logger.debug('copy_message_refresh end {}'.format(copy_message_refresh ))
    ...
    def merge_refresh_new(self, task):
        ...
        #logger.debug("refresh_router_key :{}|{}".format(session_id,db_key))
